...
export function FilterValue<TData, TType extends ColumnDataType>({ filter, column, actions, strategy, locale }: FilterValueProps<TData, TType>) {
	return (
		<Popover>
...
export function FilterValueDisplay<TData, TType extends ColumnDataType>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, TType>) {
	switch (column.type) {
		case "option":
			return <FilterValueOptionDisplay filter={filter as FilterModel<"option">} column={column as Column<TData, "option">} actions={actions} locale={locale} />
		case "multiOption":
			return <FilterValueMultiOptionDisplay filter={filter as FilterModel<"multiOption">} column={column as Column<TData, "multiOption">} actions={actions} locale={locale} />
		case "date":
		...
		case "number":
			return <FilterValueNumberDisplay filter={filter as FilterModel<"number">} column={column as Column<TData, "number">} actions={actions} locale={locale} />
		default:
			return null
	}
}
...
export function FilterValueDateDisplay<TData>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, "date">) {
	...
	const formattedRangeStr = formatDateRange(new Date(filter.values[0]), new Date(filter.values[1]))
...
export function FilterValueTextDisplay<TData>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, "text">) {
	if (!filter) return null
	if (filter.values.length === 0 || filter.values[0].trim() === "") return <EllipsisIcon className="size-4" />
...
export function FilterValueController<TData, TType extends ColumnDataType>({ filter, column, actions, strategy, locale = "en" }: FilterValueControllerProps<TData, TType>) {
	switch (column.type) {
		case "option":
			return <FilterValueOptionController filter={filter as FilterModel<"option">} column={column as Column<TData, "option">} actions={actions} strategy={strategy} locale={locale} />
		case "multiOption":
			return (
				<FilterValueMultiOptionController filter={filter as FilterModel<"multiOption">} column={column as Column<TData, "multiOption">} actions={actions} strategy={strategy} locale={locale} />
			)
		case "date":
			return <FilterValueDateController filter={filter as FilterModel<"date">} column={column as Column<TData, "date">} actions={actions} strategy={strategy} locale={locale} />
		case "text":
			return <FilterValueTextController filter={filter as FilterModel<"text">} column={column as Column<TData, "text">} actions={actions} strategy={strategy} locale={locale} />
		case "number":
			return <FilterValueNumberController filter={filter as FilterModel<"number">} column={column as Column<TData, "number">} actions={actions} strategy={strategy} locale={locale} />
		default:
			return null
	}
}
...
const OptionItem = memo(function OptionItem({ option, onToggle }: OptionItemProps) {
	...
	const handleSelect = useCallback(() => {
		onToggle(value.toString(), !selected)
	}, [onToggle, value, selected])

	return (
		<CommandItem key={value} onSelect={handleSelect} className="group flex items-center justify-between gap-1.5">
			<div className="flex items-center gap-1.5">
				<Checkbox checked={!!selected} className="mr-1 opacity-0 data-[state=checked]:opacity-100 group-data-[selected=true]:opacity-100 dark:border-ring" />
				{Icon && (isValidElement(Icon) ? Icon : <Icon className="size-4 text-primary" />)}
				<span>
					{label}
					<sup className={cn(count == null && "hidden", "ml-0.5 text-muted-foreground tabular-nums tracking-tight", count === 0 && "slashed-zero")}>
						{typeof count === "number" && count > 0 ? (count < 100 ? count : "100+") : ""}
					</sup>
				</span>
			</div>
		</CommandItem>
...
export function FilterValueOptionController<TData>({ filter, column, actions, locale = "en" }: FilterValueControllerProps<TData, "option">) {
	...
	const initialOptions = useMemo(() => {
		...
		return column.getOptions().map(o => ({
			...o,
			selected: filter?.values.includes(o.value.toString()),
			initialSelected: filter?.values.includes(o.value.toString()),
			count: counts?.get(o.value.toString()) ?? 0,
		}))
	}, [])
	...
	useEffect(() => {
		setOptions(prev => prev.map(o => ({ ...o, selected: filter?.values.includes(o.value.toString()) })))
	}, [filter?.values])
...
export function FilterValueMultiOptionController<TData>({ filter, column, actions, locale = "en" }: FilterValueControllerProps<TData, "multiOption">) {
	...
	const initialOptions = useMemo(() => {
		const counts = column.getFacetedUniqueValues()
		return column.getOptions().map(o => {
			const selected = filter?.values.includes(o.value.toString())
			return {
				...o,
				selected,
					...
					<div className="flex w-full flex-col">
						<Tabs value={isNumberRange ? "range" : "single"} onValueChange={v => changeType(v as "single" | "range")}>
							...
							<TabsContent value="single" className="mt-4 flex flex-col gap-4">
								{minMax && <Slider value={[values[0]]} onValueChange={value => changeNumber(value)} min={sliderMin} max={sliderMax} step={1} aria-orientation="horizontal" />}
