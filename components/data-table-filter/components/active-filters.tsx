	...
	return (
		<>
			{filters.map(filter => {
				...
				return <ActiveFilter key={`active-filter-${filter.columnId}`} filter={filter} column={column} actions={actions} strategy={strategy} locale={locale} />
			})}
		</>
	)
}
...
export function ActiveFilter<TData, TType extends ColumnDataType>({ filter, column, actions, strategy, locale = "en" }: ActiveFilterProps<TData, TType>) {
	return (
		<div className="flex h-7 items-center rounded-2xl border border-border bg-background text-xs shadow-xs">
			...
			<Separator orientation="vertical" />
			<Button variant="ghost" className="h-full w-7 rounded-none rounded-r-2xl text-xs" onClick={() => actions.removeFilter(filter.columnId)}>
...
export function ActiveFiltersMobileContainer({ children }: { children: React.ReactNode }) {
	...
	return (
		<div className="relative w-full overflow-x-hidden">
			{/* Left blur effect */}
			{showLeftBlur && <div className="fade-in-0 pointer-events-none absolute top-0 bottom-0 left-0 z-10 w-16 animate-in bg-gradient-to-r from-background to-transparent" />}
			...
			{/* Right blur effect */}
			{showRightBlur && <div className="fade-in-0 pointer-events-none absolute top-0 right-0 bottom-0 z-10 w-16 animate-in bg-gradient-to-l from-background to-transparent " />}
		</div>
	)
}
