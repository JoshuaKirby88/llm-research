...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: React.ComponentProps<typeof DayPicker>) {
	return (
		<DayPicker
			...
			components={{
				IconLeft: ({ className, ...props }) => <ChevronLeftIcon className={cn("size-4", className)} {...props} />,
				IconRight: ({ className, ...props }) => <ChevronRightIcon className={cn("size-4", className)} {...props} />,
			}}
			{...props}
		/>
	)
