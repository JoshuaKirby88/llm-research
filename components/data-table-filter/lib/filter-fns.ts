...
export function dateFilterFn<TData>(inputData: Date, filterValue: FilterModel<"date">) {
	...
	if (dateFilterOperators[filterValue.operator].target === "single" && filterValue.values.length > 1) throw new Error("Singular operators require at most one filter value")

	if (filterValue.operator in ["is between", "is not between"] && filterValue.values.length !== 2) throw new Error("Plural operators require two filter values")
