...
export type Column<TData, TType extends ColumnDataType = any, TVal = unknown> = ColumnConfig<TData, TType, TVal> & ColumnProperties<TData, TVal> & ColumnPrivateProperties<TData, TVal>
...
export type NumberFilterOperator = "is" | "is not" | "is less than" | "is greater than or equal to" | "is greater than" | "is less than or equal to" | "is between" | "is not between"
...
export type FilterModelUnion = {
	[K_Type in ColumnDataType]: {
		columnId: string
		type: K_Type
		operator: FilterOperators[K_Type]
		values: FilterValues<K_Type>
	}
}[ColumnDataType]
...
export type FilterOperatorDetails<OperatorValue, T extends ColumnDataType> = FilterOperatorDetailsBase<OperatorValue, T> &
	({ singularOf?: never; pluralOf?: never } | { target: "single"; singularOf: FilterOperators[T]; pluralOf?: never } | { target: "multiple"; singularOf?: never; pluralOf: FilterOperators[T] }) &
